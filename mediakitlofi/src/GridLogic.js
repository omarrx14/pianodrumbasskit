...
export const Gridtest = ({ slots
    ...
    useEffect(() => {
        ...
        const handleMouseDown = (event) => {
            ...
            const y = event.clientY - rect.top;
            // Calcula la posición del mouse relativa al grid
            const { rowIndex, colIndex } = calculateNotePosition({ clientX: x, clientY: y });

            // Intenta encontrar una nota en la posición calculada
            const note = findNoteAtPosition(rowIndex, colIndex);

            if (note) {
                // Si se encuentra una nota, actualiza el estado para indicar que la nota está seleccionada
                // y prepara para moverla
                dispatch(selectNote({ noteId: note.id }));
                setIsDragging(true); // Establece isDragging a true para habilitar el movimiento en handleMouseMove
                initialPositionRef.current = { rowIndex, colIndex }; // Guarda la posición inicial para comparaciones futuras
            }
        };
        ...
        const handleMouseMove = (event) => {
            ...
            // Actualiza la posición visual de la nota temporalmente
            tempNotePosition.current = { rowIndex, colIndex };
        };
        ...
        const handleMouseUp = () => {
            ...
            const { rowIndex, colIndex } = tempNotePosition.current;
            dispatch(updateNotePosition({
                noteId: selectedNote.id,
    ...
    }, [dispatch, isDragging, selectedNote, updateNotePosition, notes]); // Asegúrate de incluir todas las dependencias necesarias
...
const Gridtest1 = ({ slots }: GridProps): JSX.Element => {
    ...
    const handleMouseMoveOnGrid = (event) => {
        ...
        // Actualiza la posición visual de la nota temporalmente
        // Esto podría implicar redibujar la nota en su nueva posición sin actualizar el estado global
        initialPositionRef(rowIndex, colIndex, selectedNote);
    };
    ...
    useEffect(() => {
        ...
        const handleMouseDown = (event) => {
            ...
            // Verifica si el clic fue sobre una nota existente
            const clickedNote = notes.find(note => note.rowIndex === rowIndex && note.colIndex === colIndex);

            if (clickedNote) {
                // Configura el estado para reflejar que una nota fue seleccionada
                dispatch(selectNote(clickedNote.id));

                // Establece el estado inicial para el arrastre
                setDragStart({
                    startX: x,
                    startY: y,
                    noteStartX: clickedNote.x, // Asegúrate de tener esta información, o ajusta según tu estructura de datos
                    noteStartY: clickedNote.y, // Ajusta según tu estructura de datos
                    noteId: clickedNote.id // Almacenar el ID de la nota para referencia futura
                });
        ...
        const handleMouseMove = (event) => {
            if (isDragging && selectedNote) {
                ...
                dispatch(updateNotePosition({ noteId: selectedNote.id, newRowIndex, newColIndex }));
            }
